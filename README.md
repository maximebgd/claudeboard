...
<p align="center">
  <video src="https://github.com/user-attachments/assets/1a63cf16-79e9-42f5-8420-52bc92a17b5c" controls muted></video>
</p>
...
│   ├── skills.ts          # list/get/write/create/deleteSkill (previous version archived via backups.ts)
│   ├── projects.ts        # listProjects · listSessions · getSession · JSONL block normalization
│   ├── mdEntries.ts       # agents & commands: list/get/write/create/delete (.md, nested slugs = namespaces, versioned)
│   ├── configFiles.ts     # read/write/reset/deleteConfigFile: settings, CLAUDE.md, keybindings (validated, versioned)
│   ├── backups.ts         # version history (data/backups/<target>, outside ~/.claude) for config + skills/agents/commands
...
│   ├── rateLimits.ts      # getRateLimits: read-only Claude.ai usage limits from the statusline cache
...
- **Analytics dashboard (`/`)** — aggregates every JSONL transcript in one pass: KPIs (projects, sessions, messages, tokens, estimated cost), an **activity panel** (`ActivityPanel`) toggling between a 12-month heatmap and a per-day message curve with a consecutive-day **streak**, a model donut (`ModelDonut`) with IN/OUT counts, tokens & cost per model, cost per project, an hourly distribution of session starts (local time), most-used tools/skills, pinned sessions, and session stats. A `RangeSelector` filters the window (all / 30d / 7d, plus a single calendar button grouping a given month, a custom range, and a **subscription billing cycle** aligned to your plan's anniversary day); the relevant KPIs show a **velocity delta** vs the previous period of equal length (N vs N-1). The clickable **Cost** KPI (`CostStatCard`) toggles between estimated usage cost and net subscription savings; a `SubscriptionCard` compares usage cost against your Claude plan price. A `UsageBanner` in the header surfaces your Claude.ai **usage limits** (rolling 5-hour / 7-day windows: percent consumed + reset countdown), read **out of real time** from the statusline cache — this requires a small statusline tweak (see [Features → Usage limits](docs/en/fonctionnalites.md)); without it the gauges stay empty and link to the setup docs.
...
- **Skills (`/skills`)** — list, preview, **edit**, **create** and **delete** each `~/.claude/skills/*/SKILL.md` (YAML frontmatter + markdown body). Every save archives the previous version in the restorable **Versions** panel (stored **outside** `~/.claude`, git-style diff); deletes move the folder to the trash.
...
- **Config (`/config/*`)** — **Preferences** (claudeboard's own settings: write permissions, estimation pricing, subscription, cost-card display, **language** FR/EN), **Settings** (edit `settings.json` / `settings.local.json`, live-validated, restorable **Versions** panel, reset), **Hooks** (grouped by event, **editable** hooks block of `settings.json`), **Agents** & **Commands** (list/preview/edit/create/delete, nested folders = namespaces, same restorable **Versions** panel as skills), **Dependency graph** (read-only cross-references), **global CLAUDE.md** editor (create/reset/delete), **MCP servers** (read-only, `env` masked), **Plugins & Marketplaces** (read-only, install stays in the CLI), **Keybindings** (table + JSON editor, create/reset/delete) and **Trash** (restore or permanently delete items removed from the app). An educational `.claude` tree lives under `/docs/structure`.
...
**Nothing is ever lost — one version history + a trash, both outside `~/.claude`:**

- **Edit anything** — a skill / agent / command, or a config file (settings, hooks, CLAUDE.md, keybindings) → the previous version is archived **outside** `~/.claude`, in claudeboard's `data/backups/`, exposed as the restorable **Versions** panel (git-style diff, "Current" badge, capped at 10 per target). No more `.bak` files cluttering `~/.claude`.
...
flowchart TD
    ...
    subgraph DATA["data/ (outside CLAUDE_DIR)"]
      ...
      BACKUPS["backups/ — version history (config + skills/agents/commands)"]
    ...
    GUARD -->|read| SKILLS
    GUARD -->|write| SKILLS
    LIB -.->|archive previous version| BACKUPS
...
Contributions are welcome! Please read [`CONTRIBUTING.md`](./CONTRIBUTING.md) to get started, and note the [Code of Conduct](./CODE_OF_CONDUCT.md). Found a security issue? See [`SECURITY.md`](./SECURITY.md) — please report it privately.
