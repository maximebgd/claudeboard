 ...
 * fichier via `writeConfigFile` (qui archive d'abord la version courante) et est donc
 * verrouillée par la permission `modify` de la ressource cible.
 */
...
export async function POST(req: Request) {
  ...
  // Restaurer réécrit tout le fichier → permission `modify` de la ressource cible.
  if (!(await isAllowed(configResource(target), "modify"))) {
    return NextResponse.json(
      { error: "Restoring is not allowed — enable it in Preferences." },
      { status: 403 }
    );
  }
