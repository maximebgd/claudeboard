...
| i18n | Bilingue EN/FR maison (cœur isomorphe, français par défaut) |
...
claudeboard/
├── lib/
│   ├── claude.ts          # CLAUDE_DIR + safeResolve (garde anti-traversée) — tout reste confiné
│   ├── format.ts          # formatteurs nombre/monnaie/date/taille sensibles à la locale
...
│   ├── export.ts          # session/projet → Markdown ou HTML autonome (lecture seule)
│   ├── graph.ts           # getDependencyGraph : références croisées skills/agents/commandes (lecture seule)
...
│   ├── rateLimits.ts      # getRateLimits : limites d'usage Claude.ai (lecture seule) depuis le cache du statusline
│   ├── docs.ts            # listDocs · getDoc : rend les fichiers .md de docs/ sur /docs
│   ├── i18n.ts            # getT() (serveur) : lit la langue du store → { locale, t }
│   └── i18n/              # core.ts (translate/tPlural isomorphes) · translations.ts (dico plat EN/FR)
├── app/
...
│   ├── search/            # recherche full-text en lecture seule sur tous les transcripts
│   ├── config/            # preferences (permissions + tarifs + abonnement + langue) · settings · hooks ·
│   │                       #   claude-md · agents · commands · graph · mcp · plugins · keybindings · trash
│   ├── docs/              # layout · page · [slug] (rend docs/*.md) · structure (arbre .claude)
│   ├── api/               # skills · md · config-file · hooks · projects (écritures gated) · store · search · export · trash
...
- **Dashboard analytics (`/`)** — agrège tous les transcripts JSONL en un seul passage : KPI (projets, sessions, messages, tokens, coût estimé), un **panneau d'activité** (`ActivityPanel`) qui bascule entre une heatmap sur 12 mois et une courbe des messages par jour avec un **streak** de jours consécutifs, camembert des modèles (`ModelDonut`) avec comptes IN/OUT, tokens & coût par modèle, coût par projet, distribution horaire des débuts de session (heure locale), outils/skills les plus utilisés, sessions épinglées et stats de session. Un `RangeSelector` filtre la fenêtre (tout / 30 j / 7 j, plus un unique bouton calendrier regroupant un mois donné, une plage libre et un **cycle de facturation d'abonnement** calé sur le jour d'anniversaire de votre plan) ; les KPI concernés affichent un **delta de vélocité** vs la période précédente de même durée (N vs N-1). La carte KPI **Coût** cliquable (`CostStatCard`) bascule entre coût d'usage estimé et économie nette d'abonnement ; une `SubscriptionCard` compare le coût d'usage au prix de votre plan Claude. Un `UsageBanner` dans l'en-tête affiche vos **limites d'usage** Claude.ai (fenêtres glissantes 5 h / 7 j : % consommé + compte à rebours de reset), lues **hors temps réel** du cache du statusline — cela demande un petit réglage du statusline (voir [Fonctionnalités → Limites d'usage](docs/fr/fonctionnalites.md)) ; sans lui les jauges restent vides et pointent vers la doc de configuration.
...
flowchart TD
    subgraph CLIENT["Client (navigateur)"]
      UI["Dashboard · éditeurs · recherche · RangeSelector"]
    end
